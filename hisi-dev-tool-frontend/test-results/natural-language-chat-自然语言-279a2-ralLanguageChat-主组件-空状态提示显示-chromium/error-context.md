...
- Name: natural-language-chat.spec.ts >> 自然语言诊断组件测试 >> NaturalLanguageChat 主组件 >> 空状态提示显示
- Location: e2e\natural-language-chat.spec.ts:64:5
...
Locator: locator('text=选择一个诊断会话').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=选择一个诊断会话').first()
