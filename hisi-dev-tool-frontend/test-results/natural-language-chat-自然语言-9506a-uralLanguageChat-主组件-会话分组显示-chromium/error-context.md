...
- Name: natural-language-chat.spec.ts >> 自然语言诊断组件测试 >> NaturalLanguageChat 主组件 >> 会话分组显示
- Location: e2e\natural-language-chat.spec.ts:37:5
...
Locator: locator('text=进行中').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=进行中').first()
