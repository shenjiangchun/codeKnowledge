...
- Name: integration.spec.ts >> HiSi DevTool v4.1 集成测试 >> 侧边栏导航 - 语义搜索入口
- Location: e2e\integration.spec.ts:27:3
...
Locator: locator('.el-menu-item[index="/search"], li:has-text("语义搜索")').first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('.el-menu-item[index="/search"], li:has-text("语义搜索")').first()
