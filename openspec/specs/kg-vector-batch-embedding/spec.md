# KG Vector Batch Embedding

## Purpose
Provide batch embedding generation capability via `UnifiedEmbeddingService.generateEmbeddings(List<String>)`, sending multiple texts in a single `/embeddings` API call to reduce HTTP round trips by ~95% (from ~10K to ~500 calls for 5051 methods).
