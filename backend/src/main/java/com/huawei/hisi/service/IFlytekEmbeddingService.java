...
import com.huawei.hisi.config.IFlytekConfig;
...
/**
 * 科大讯飞 Embedding 服务（已废弃）
 * 功能已迁移至 UnifiedEmbeddingService。
 * 仅在 iflytek.enabled=true 时加载（默认 false），保留向后兼容。
 *
...
@ConditionalOnProperty(name = "iflytek.enabled", havingValue = "true")
public class IFlytekEmbeddingService {
    ...
    public float[] generateEmbedding(String text) {
        ...
        for (int attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                log.debug("[讯飞] 调用向量生成API: model={}, textLength={}, attempt={}/{}",
                ...
                if (e.getStatusCode().value() == 429 && attempt < MAX_RETRIES) {
                    ...
                    log.warn("[讯飞] 向量生成限流(429)，第{}次重试，等待{}ms", attempt + 1, delay);
                    sleepQuietly(delay);
                    continue;
                }
                log.error("[讯飞] 向量生成失败: {}", e.getMessage());
                throw new RuntimeException("讯飞向量生成失败: " + e.getMessage(), e);
            } catch (Exception e) {
                log.error("[讯飞] 向量生成失败: {}", e.getMessage());
                throw new RuntimeException("讯飞向量生成失败: " + e.getMessage(), e);
            }
        }
        throw new RuntimeException("讯飞向量生成失败: 超过最大重试次数");
    }
