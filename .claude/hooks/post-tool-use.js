...
const username = sanitize(getGitUser(process.cwd()));
...
const oldUsername = sanitize(os.userInfo().username);
const oldSessionFile = path.join(sessionDir, `${oldUsername}-ai-session-${hostname}.json`);
if (fs.existsSync(oldSessionFile) && !fs.existsSync(sessionFile)) { try { const old = JSON.parse(fs.readFileSync(oldSessionFile, 'utf8')); old.developer = getGitUser(projectRoot); fs.writeFileSync(sessionFile, JSON.stringify(old, null, 2), 'utf8'); } catch {} }
let session = {
  developer: getGitUser(projectRoot), hostname,
  model: detectModel(),
...
if (idx >= 0) {
  ...
  session.pendingFiles[idx].aiTool = 'claude-code';
  session.pendingFiles[idx].model = session.model;
  session.pendingFiles[idx].lastModified = new Date().toISOString();
} else {
  session.pendingFiles.push({ path: filePath, tool: toolName, aiTool: 'claude-code', model: session.model, operations: 1, timestamp: new Date().toISOString(), lastModified: new Date().toISOString() });
