...
- Name: integration.spec.ts >> 智能诊断页面 >> 诊断按钮存在
- Location: e2e\integration.spec.ts:54:3
...
Locator: locator('button:has-text("诊断"), button:has-text("开始"), button:has-text("Diagnose")').first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('button:has-text("诊断"), button:has-text("开始"), button:has-text("Diagnose")').first()
