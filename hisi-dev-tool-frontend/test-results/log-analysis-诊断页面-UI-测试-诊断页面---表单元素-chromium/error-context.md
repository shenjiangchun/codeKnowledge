...
- Name: log-analysis.spec.ts >> 诊断页面 UI 测试 >> 诊断页面 - 表单元素
- Location: e2e\log-analysis.spec.ts:240:3
...
Locator: locator('h2, h3, .el-card__header').filter({ hasText: /诊断|智能/ }).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('h2, h3, .el-card__header').filter({ hasText: /诊断|智能/ }).first()
